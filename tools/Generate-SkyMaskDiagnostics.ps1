param(
    [string]$OutputDirectory = 'app\build\reports\sky-mask-diagnostics\urban-window-30'
)
...
$env:ASTROPHOTO_SKY_MASK_DIAGNOSTICS_OUTPUT_DIR = $outputRoot
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Sky-mask replay diagnostics failed with exit code $LASTEXITCODE"
    }
} finally {
    Pop-Location
}

Write-Host "Generated deterministic replay-only sky-mask diagnostics in $outputRoot"
