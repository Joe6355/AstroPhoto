...
fun buildExposureRecommendation(
    ...
    when (testShot.status) {
        ...
        TestShotStatus.NORMAL -> {
            explanation += "ISO оставить: $currentIso. Выдержку оставить: " +
    ...
    if (
        goal == ShootingGoal.STARS &&
        testShot.status !in setOf(TestShotStatus.OVEREXPOSED, TestShotStatus.TOO_BRIGHT) &&
        (testShot.starFocus?.measuredStars ?: 0) < MIN_ASSISTANT_STARS
    ) {
        val increasedExposure = moveExposure(exposure, exposureRange, steps = 1, brighter = true)
        if (increasedExposure != null && increasedExposure > exposure) {
            val previous = exposure
            exposure = increasedExposure
            explanation += "Найдено мало звёзд (${testShot.starFocus?.measuredStars ?: 0}): " +
                "увеличиваем выдержку с ${formatAssistantExposure(previous)} до " +
                "${formatAssistantExposure(exposure)}."
        } else {
            explanation += "Найдено мало звёзд (${testShot.starFocus?.measuredStars ?: 0}); " +
                "выдержка уже на доступном максимуме."
        }
    }
...
private const val MIN_ASSISTANT_STARS = 3
...
private fun nextIso(
    ...
    val values = (cameraIsoPresets(range) + current)
        .distinct()
        .sorted()
        .filter { it in range }
    return if (brighter) {
