package com.joe6355.astrophoto.processing.jpeg.v2.completion

import com.joe6355.astrophoto.JpegStackResult
import com.joe6355.astrophoto.formatTotalExposure
...
internal fun automaticProfileSuccessStatus(
    ...
    created.statistics?.let { statistics ->
        append(" • отброшено: ${statistics.droppedFrames}")
        statistics.totalExposureNs?.let { append(" • сумма: ${formatTotalExposure(it)}") }
        statistics.medianFwhm?.let {
            append(String.format(java.util.Locale.getDefault(), "\nFWHM: %.2f px", it))
        }
        statistics.meanAlignmentConfidence?.let {
            append(String.format(java.util.Locale.getDefault(), " • alignment: %.0f%%", it * 100f))
        }
        append(" • смазано: ${statistics.blurredFrames} • пересвечено: ${statistics.clippedFrames}")
    }
