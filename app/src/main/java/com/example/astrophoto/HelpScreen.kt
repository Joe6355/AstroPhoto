...
val ASTROPHOTO_HELP_SECTIONS = listOf(
    ...
    HelpSection(
        ...
        "Приложение выбирает заднюю камеру с ручным управлением и использует " +
            "диапазоны выдержки и ISO, которые сообщает Camera2. Для Samsung " +
            "и Sony учитываются поддерживаемые vendor-диапазоны, для Google " +
            "Pixel — публичные диапазоны Camera2. Фактические параметры " +
            "проверяются по результату обычного снимка."
    ),
    HelpSection(
        HelpTopic.ISO,
        "ISO",
        "Чем выше ISO, тем светлее кадр, но больше шума. Доступный диапазон " +
            ...
            "фактически доступными значениями."
    ),
    HelpSection(
        HelpTopic.EXPOSURE,
        "Выдержка",
    ...
    HelpSection(
        ...
        "Для звёзд обычно нужен фокус на бесконечность. Если кадр выглядит " +
            ...
            "фокусом ∞, ручным фокусом и автофокусом."
    ),
    HelpSection(
        HelpTopic.RAW,
        "JPEG и RAW/DNG",
    ...
    HelpSection(
        ...
        "Доступны серии от 1 до 500 кадров, включая 100, 150, 200, 300 и 500. " +
            "Можно задать паузу между снимками и таймер старта. Для большой " +
            "серии заранее проверьте свободное место и питание телефона."
    ),
    HelpSection(
        HelpTopic.STACKING,
        "Stacking",
    ...
    HelpSection(
        ...
        "Перед stacking можно отметить бракованные кадры. Автоматический отбор " +
            ...
            "недостаточно, приложение сообщит об этом. Результаты находятся " +
            "в разделе Processed выбранной сессии."
    ),
    HelpSection(
        HelpTopic.TIMELAPSE,
        "MP4-таймлапс",
