...
import com.example.astrophoto.SessionFrame
...
class ProfileAnalysisCheckpointStore private constructor(
    ...
    fun read(frame: SessionFrame, captureIndex: Int): CheckpointedProfileAnalysis? =
        runCatching {
            ...
            if (!file.isFile || file.length() !in 1..MAX_CHECKPOINT_BYTES) return@runCatching null
            DataInputStream(BufferedInputStream(file.inputStream())).use { input ->
                if (input.readInt() != MAGIC || input.readInt() != VERSION) return@use null
                if (input.readUTF() != fingerprint || input.readUTF() != frame.key) return@use null
                if (input.readLong() != frame.sizeBytes || input.readLong() != frame.createdAtMillis) {
                    return@use null
                }
                val analysis = input.readFrameAnalysis()
                val skyMask = input.readSkyMaskResult()
                val observation = input.readSensorObservation()
                if (observation.originalCaptureIndex != captureIndex ||
    ...
    companion object {
        ...
        private fun DataInputStream.readFrameAnalysis(): FrameAnalysis {
            ...
            return FrameAnalysis(
                ...
                readFloat(), readFloat(), readBoolean()
            )
        }
